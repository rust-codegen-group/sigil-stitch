...
pub(crate) fn lower(type_name: &TypeName) -> Result<CodeBlock, SigilStitchError> {
    match type_name {
        TypeName::Importable {
            ...
            "qualified type references have no Zsh representation",
        )),
        TypeName::Importable { .. } | TypeName::Primitive(_) | TypeName::Raw(_) => {
            Ok(terminal(type_name))
        }
        TypeName::StringLiteral(_) => {
            Err(unsupported("Zsh has no string singleton type expression"))
        }
        _ => Err(unsupported(
            "compound type references have no Zsh representation",
        )),
    }
}

#[cfg(test)]
assert_string_literal_rejection!("zsh", "Zsh has no string singleton type expression");
