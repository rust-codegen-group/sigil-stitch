//! TypeScript-owned field grammar.
...
const CONCRETE_CAPABILITIES: &[FieldCapability] = &[
...
const CONTRACT_CAPABILITIES: &[FieldCapability] = &[
    FieldCapability::ExplicitType,
    FieldCapability::ReadOnly,
    FieldCapability::OptionalPresence,
];
...
pub(crate) fn collect_validation_errors(
    ...
    let contract = matches!(
        fields.context(),
        FieldContext::Direct(DeclarationContext::InterfaceMember)
            | FieldContext::TypeMember(TypeKind::Interface | TypeKind::Trait)
    );
    for field in fields.fields() {
        let private_name = field.name().starts_with('#');
        let visibility_is_valid = if private_name {
            !contract && field.modifiers().visibility == Visibility::Inherited
        } else if contract {
        ...
        if !visibility_is_valid {
            errors.push(SigilStitchError::InvalidField {
                ...
                reason: "TypeScript does not support this field visibility in the selected context"
                    .to_string(),
            });
        }
        let key = property_key(field.name());
        if !contract && key.as_deref() == Some("constructor") {
            errors.push(SigilStitchError::InvalidField {
                language: lang.file_extension().to_string(),
                field_name: field.name().to_string(),
                context: fields.context(),
                reason: "TypeScript class fields cannot be named constructor".to_string(),
            });
        }
        if !contract
            && field.modifiers().is_static
            && !private_name
            && key.as_deref() == Some("prototype")
        {
            errors.push(SigilStitchError::InvalidField {
                language: lang.file_extension().to_string(),
                field_name: field.name().to_string(),
                context: fields.context(),
                reason: "TypeScript static class fields cannot be named prototype".to_string(),
            });
        }
