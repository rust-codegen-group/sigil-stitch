//! Haskell-owned record-field grammar.
    ...
    for field in fields.fields() {
        if field.name() == "_" {
            errors.push(SigilStitchError::InvalidField {
                ...
                reason: "Haskell record fields require a named selector".to_string(),
            });
        }
        if field.modifiers().visibility != Visibility::Inherited {
            errors.push(SigilStitchError::InvalidField {
                ...
                field_name: field.name().to_string(),
                context: fields.context(),
                reason: "Haskell record fields do not have declaration visibility modifiers"
                    .to_string(),
            });
        }
        if matches!(
        ...
        ) && !field.doc().is_empty()
        {
            errors.push(SigilStitchError::InvalidField {
                ...
                field_name: field.name().to_string(),
                context: fields.context(),
                reason: "record-constructor payload fields do not support standalone documentation blocks"
                    .to_string(),
            });
        }
    }
