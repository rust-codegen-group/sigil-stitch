//! TypeScript-owned computed-property grammar.
...
use crate::lang::field_lowering::typescript::{is_valid_property_name, property_key};
...
pub(crate) fn collect_validation_errors(
    ...
    if !is_valid_property_name(property.name()) {
        invalid(
            "property name is not a valid TypeScript property name",
            errors,
        );
    }
    let private_name = property.name().starts_with('#');
    let valid_visibility = if private_name {
        property.modifiers().visibility == Visibility::Inherited
    } else {
        matches!(
    ...
    if !valid_visibility {
        invalid(
            "TypeScript does not support this property visibility in a concrete type",
            errors,
        );
    }
    if property_key(property.name()).as_deref() == Some("constructor") {
        invalid("TypeScript accessors cannot be named constructor", errors);
    }
    ...
    {
        invalid(
            "TypeScript static accessors cannot be named prototype",
            errors,
        );
    }
    ...
    {
        invalid(
            "setter parameter is not a valid TypeScript binding identifier",
            errors,
        );
    }
}
...
fn emit_prefix(block: &mut CodeBlockBuilder, lang: &TypeScript, property: &PropertySpec) {
    block.add(
        "%L",
